...
document.addEventListener("keydown", function(event) {
    if (event.keyCode==65 || event.keyCode == 37) {      // left key
        if (parseInt(rod1.style.left) > border) {
            rod1.style.left = parseInt(rod1.style.left) - movement + "px";
            rod2.style.left = rod1.style.left;
        }
    } else if (event.keyCode==68 || event.keyCode==39) { // right key
...
function storeData(score, winningRod) {
    ...
    alert(winningRod + " wins with the score of "+ (score * 100)+"\nMax Score: "+(highScore * 100));
}